...
# Разметка цены с 17.08.2026: <b>28,</b><small>40 ₽/шт</small> — рубли крупно,
# копейки и единица мелко (price_split). Забираем оба куска и склеиваем.
RE_PRICE = re.compile(r'<p class="pd-price">(.*?)</p>', re.S)
...
def text(s):
    ...
    """
    # Тег заменяем ПРОБЕЛОМ, а не пустой строкой: в заголовке раздела рядом
    # с именем стоит счётчик (<h1>Донской кирпич<span>47 видов</span></h1>),
    # и без разделителя в индекс попадало «Донской кирпич47 видов».
...
def build():
    ...
    for f in sorted((BASE / "tovar").glob("*.html")):
        ...
        # «28,» + «40 ₽/шт» — между кусками пробела быть не должно
        price = text(pm.group(1)).replace(", ", ",").replace(" /", "/") if pm else ""
        unit = ""
        ...
        if keys:
            rec["k"] = " ".join(keys)
            # Последняя крошка — это завод («Тербунский гончар») или форма
            # плитки: она и печатается мелкой строкой под именем в выдаче
            # поиска. Раньше туда шло всё «k» целиком и обрезалось многоточием.
            rec["z"] = keys[-1]
        items.append(rec)
