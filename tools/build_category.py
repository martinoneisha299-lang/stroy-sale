...
{chr(10).join(lanes)}
    ...
    <section class="section" id="calc" aria-label="Калькулятор кирпича">
      <div class="wrap">
        ...
        <div class="calc">
          ...
          <div class="calc-out">
            <p class="tag">Понадобится примерно</p>
            <p class="calc-num" id="calcQty" aria-live="polite">—</p>
            <p class="caption" id="calcCap">Введите площадь стен — посчитаем
              штуки с запасом.</p>
  ...
  <script>
    ...
    var calcCap = document.getElementById('calcCap');

    function recalc() {
      var area = (parseFloat(cWall.value) || 0) - (parseFloat(cOpen.value) || 0);
      if (area <= 0) {
        calcQty.textContent = '—';
        calcCap.textContent = 'Введите площадь стен — посчитаем штуки с запасом.';
        return;
      }
      var qty = Math.ceil(area * parseFloat(cFmt.value) * 1.05 / 10) * 10;
      calcQty.textContent = qty.toLocaleString('ru-RU') + ' шт';
      calcCap.textContent = 'На ' + area.toLocaleString('ru-RU') +
        ' м² кладки, запас 5% включён.';
    }
